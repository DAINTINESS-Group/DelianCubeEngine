package intentional.analyze.cubeQueryGenerator;
 ...
 *
 */
public class CinecubesDrillDownsQueryGenerator implements CubeQueryGenerator{
	
	private CubeManager cubeManager;
	
	public CinecubesDrillDownsQueryGenerator(CubeManager cubeManager) {
	...
	private String[][] getChildrenValues(String schemaName,String tableName,String child,String currentLevel,String currentLevelValue,String connectionType) {
		...
		String SQLQuery = "SELECT DISTINCT " + tableName + "." + child + " FROM " + tableName + " WHERE " + whereClauseExpression + ";";
		System.out.println("The query that get the child values for the drill down query: "+SQLQuery);
		...
		for(int i = 0;i < sigmaExpressions.size();i++) {
			...
			for(int j = 0;j < gammaExpressions.size();j++) {
				...
				if(gammaDimension.equals(sigmaDimension)) {
					...
					newGamma = childToLevelByName.get(gammaExpression);
			...
			if(createDrillDown == true) {
				for(int j = 2;j < childrenValues.length;j++) {
					for(int k = 0;k < childrenValues[k].length;k++) {
						...
						String sigma = "Sigma:" + sigmaDimension + "." + childToLevelByName.get(expression) + "=" + "\'" + childrenValues[j][k] + "\'";

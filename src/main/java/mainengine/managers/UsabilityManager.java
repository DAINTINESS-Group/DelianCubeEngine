...
public class UsabilityManager {
    ...
    /**
     * Sets the current query q^n and wires up the {@link QueryHistoryManager}
     * so that {@link #checkUsability()} and
     * {@link #executeCubeQueryWithUsability(CubeQuery)} have access to the
     * full query history.
     * <p>
     * The base query q^b is automatically resolved as the last query
     * in the history.
     *
     * @param newQuery       the current query q^n (not yet in history)
     * @param historyManager the session's query history (may be null
     *                       if no history exists yet)
     */
    public void setQueries(CubeQuery newQuery, QueryHistoryManager historyManager, CubeBase cubeBase) {
        this.newQuery = newQuery;
        this.historyManager = historyManager;
        this.baseQuery = (historyManager != null) ? historyManager.getLastQuery() : null;
    ...
    /**
     * Checks all six conditions of Theorem 9.1 (Cube Usability) and if the
     * most recent query(in history) does not qualify, moves further back through
     * the {@link QueryHistoryManager} to find the most recent cached query
     * that is usable for computing q^n.
     * <p>
     * If a usable base query is found anywhere in history, {@link #baseQuery}
     * is updated to point to it so that
    ...
    public boolean checkUsability() {
        ...
        //collect all candidate base queries from history, most-recent first
        List<CubeQuery> candidates = getCandidatesFromHistory();

        for (CubeQuery candidate : candidates) {
            if (candidate.getResult() == null) {
                continue;
            }
            //Implement checks for each candidate
            this.baseQuery = candidate;
            if (checkCondition1SameDS()
                ...
                System.out.println("[Usability] Found usable base query: "
                        + candidate.getName());
                return true;
            }
        }

        //no usable candidate found - discard it
        this.baseQuery = null;
        return false;
    }
     ...
     * oldest, so that {@link #checkUsability()} prefers the closest usable
     * cached result.
     */
    private List<CubeQuery> getCandidatesFromHistory() {
